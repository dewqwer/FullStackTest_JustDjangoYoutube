...
class PeopleCounter:
    ...
    m_frame_crop_arm_mean_lpft_set = mfsm_arm._initial_frame_crop_mean_lpft_set(
        3)
    ...
    while True:
        ...
        cm_peoplepass_flashcheck._set_width_hight_x1_y1(
            90, 230, 170-45, 30, "PeoplePassCheckAndFlashCheck")

        # m_frame_crop_peoplepass_flashcheck = cm_peoplepass_flashcheck._crop_form_m_frame(m_frame, 170, 10, 170+45, 10+260 )
        m_frame_crop_peoplepass_flashcheck = cm_peoplepass_flashcheck._crop_form_m_frame_by_cm_obj(
            m_frame.copy())

        # c_frame = cm_peoplepass_flashcheck._create_m_frame_line(c_frame, 170, 10, 170+45, 10+260,"PeoplePassCheckAndFlashCheck")
        cl_frame = cm_peoplepass_flashcheck._create_m_frame_line_by_cm_obj(
            cl_frame)

        # mfm_arm = mfm.MFrameManagement()
        m_frame_crop_arm_mean_current = mfm_arm._find_avg_value_of_m_frame(
            m_frame_crop_arm)
        mfm_peoplepass_flashcheck = mfm.MFrameManagement()
        m_frame_crop_peoplepass_flashcheck_mean_current = mfm_peoplepass_flashcheck._find_avg_value_of_m_frame(
            m_frame_crop_peoplepass_flashcheck)
    ...
    # Create frame to shown on moniter 9*9_________Start____________________________________________________________________
        ...
        mc_frame[cm_arm.y1_crop_arm_frame: cm_arm.y2_crop_arm_frame,
                 cm_arm.x1_crop_arm_frame: cm_arm.x2_crop_arm_frame] = mc_frame_crop_arm

        mc_frame_crop_peoplepass_flashcheck = cv2.cvtColor(
            m_frame_crop_peoplepass_flashcheck, cv2.COLOR_BGR2RGB)
        # mc_frame[10 : 10+260,170 : 170+45 ] = mc_frame_crop_peoplepass_flashcheck
        mc_frame[cm_peoplepass_flashcheck.y1_crop_peoplepass_flashcheck_frame: cm_peoplepass_flashcheck.y2_crop_peoplepass_flashcheck_frame,
                 cm_peoplepass_flashcheck.x1_crop_peoplepass_flashcheck_frame: cm_peoplepass_flashcheck.x2_crop_peoplepass_flashcheck_frame] = mc_frame_crop_peoplepass_flashcheck

        mc_frame = cm_arm._create_m_frame_line(
            mc_frame, cm_arm.x1_crop_arm_frame, cm_arm.y1_crop_arm_frame, cm_arm.x2_crop_arm_frame, cm_arm.y2_crop_arm_frame, "MFrameCropArm")

        mc_a_frame = c_frame.copy()
        # mc_a_frame[110 : 110+35,155 : 155+15 ] = mc_frame_crop_arm
        mc_a_frame[cm_arm.y1_crop_arm_frame: cm_arm.y2_crop_arm_frame,
                   cm_arm.x1_crop_arm_frame: cm_arm.x2_crop_arm_frame] = mc_frame_crop_arm

        mc_pf_frame = c_frame.copy()
        # mc_pf_frame[10 : 10+260,170 : 170+45 ] = mc_frame_crop_peoplepass_flashcheck
        mc_pf_frame[cm_peoplepass_flashcheck.y1_crop_peoplepass_flashcheck_frame: cm_peoplepass_flashcheck.y2_crop_peoplepass_flashcheck_frame,
                    cm_peoplepass_flashcheck.x1_crop_peoplepass_flashcheck_frame: cm_peoplepass_flashcheck.x2_crop_peoplepass_flashcheck_frame] = mc_frame_crop_peoplepass_flashcheck
        ...
        mc_sum_frame[281:281+281,
                     0:500] = cv2.cvtColor(m_frame.copy(), cv2.COLOR_BGR2RGB)  # 12
    ...
    # Create frame to shown on moniter 9*9_________End____________________________________________________________________
        ...
        # fem_obj = fem.FilterAndCheckErrorManagement()
        people_pass_flash_check_status = fem_obj._people_pass_flash_check(
            m_frame_crop_peoplepass_flashcheck_mean_current)

        if(people_pass_flash_check_status):
            print("people_pass_flash_check_status" +
                  str(people_pass_flash_check_status))
            people_pass_flash_check_status = False
            # m_frame_crop_arm_mean_lpft_current = -1
        else:
            # update value in avg arm set
            m_frame_crop_arm_mean_set = mfsm_arm._update_m_frame_crop_mean_set(
                m_frame_crop_arm_mean_current)
            # low pass filter 15 frame
            m_frame_crop_arm_mean_lpft_current = fem_obj._lowpassfilter_m_frame(
                m_frame_crop_arm_mean_set, 15)
            # filter low value
            m_frame_crop_arm_mean_lpft_current = fem_obj._filter_low_value(
                m_frame_crop_arm_mean_lpft_current, 30)
            # update value in lpft avg arm set
            m_frame_crop_arm_mean_lpft_set = mfsm_arm._update_m_frame_crop_mean_lpft_set(
                m_frame_crop_arm_mean_lpft_current)
            ...
            print("A: "+str(m_frame_crop_arm_mean_current)+" A_LPFT: "+str(m_frame_crop_arm_mean_lpft_current) +
                  " E: "+str(m_frame_crop_peoplepass_flashcheck_mean_current))
            print("status_up_down_slope_current : " +
                  str(status_up_down_slope_current))
            # update value in status_up_down_slope_set
            status_up_down_slope_set = mfsm_arm._update_status_up_down_slope(
                status_up_down_slope_current)
            # counter people
        ...
        info_2 = [
            ...
            ("[INFO] Slope Up Wave Counts: ", str(mfsm_arm.status_up_count)),
            ("[Status] Counter Lock Status: ", str(mfsm_arm.status_count_lock)),
            ("[Status] Slope Current Status: ", str(
                mfsm_arm.status_up_down_slope_current)),
            ("[Status] Slope Set Status: ", str(
                mfsm_arm.status_up_down_slope_set)),
            ("[Status] Filter Low Value Status: ",
             str(fem_obj.filter_low_value_status)),
            ("[Status] People Pass or Flash: ", str(
                fem_obj.people_pass_flash_check_status)),
            ("People Counts", people_count),
    ...
    i_mf_a.write("End : "+str(datetime.datetime.now()) +
                 "___________________________________________")
    i_mf_a_lpft.write("End : "+str(datetime.datetime.now()) +
                      "___________________________________________")
    i_mf_err.write("End : "+str(datetime.datetime.now()) +
                   "___________________________________________")
    i_suw.write("End : "+str(datetime.datetime.now()) +
                "___________________________________________")
    i_tf.write("End : "+str(datetime.datetime.now()) +
               "___________________________________________")

    s_mf_err.write("End : "+str(datetime.datetime.now()) +
                   "___________________________________________")
    s_fl.write("End : "+str(datetime.datetime.now()) +
               "___________________________________________")
    s_s.write("End : "+str(datetime.datetime.now()) +
              "___________________________________________")
    s_cl.write("End : "+str(datetime.datetime.now()) +
               "___________________________________________")
